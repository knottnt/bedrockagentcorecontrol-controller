...
// +kubebuilder:rbac:groups=bedrockagentcorecontrol.services.k8s.aws,resources=gateways,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bedrockagentcorecontrol.services.k8s.aws,resources=gateways/status,verbs=get;update;patch
...
func (rm *resourceManager) ARNFromName(name string) string {
	return fmt.Sprintf(
		"arn:aws:bedrockagentcorecontrol:%s:%s:%s",
		rm.awsRegion,
		rm.awsAccountID,
		name,
	)
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	statusCandidates := []string{"READY", "UPDATE_UNSUCCESSFUL", "FAILED"}
