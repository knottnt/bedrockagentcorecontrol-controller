...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Status.TargetID = &identifier.NameOrID

	f0, f0ok := identifier.AdditionalKeys["gatewayIdentifier"]
	if f0ok {
		r.ko.Spec.GatewayIdentifier = aws.String(f0)
	}
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["targetID"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: targetID"))
	}
	r.ko.Status.TargetID = &primaryKey
	f0, ok := fields["gatewayIdentifier"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: gatewayIdentifier"))
	}
	r.ko.Spec.GatewayIdentifier = &f0
