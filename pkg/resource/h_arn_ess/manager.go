...
// +kubebuilder:rbac:groups=bedrockagentcorecontrol.services.k8s.aws,resources=harnesses,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=bedrockagentcorecontrol.services.k8s.aws,resources=harnesses/status,verbs=get;update;patch
...
func (rm *resourceManager) ARNFromName(name string) string {
	return fmt.Sprintf(
		"arn:%s:bedrockagentcorecontrol:%s:%s:%s",
		rm.awsPartition,
		rm.awsRegion,
		rm.awsAccountID,
		name,
...
func (rm *resourceManager) IsSynced(ctx context.Context, res acktypes.AWSResource) (bool, error) {
	...
	statusCandidates := []string{"READY", "CREATE_FAILED", "UPDATE_FAILED", "DELETE_FAILED"}
